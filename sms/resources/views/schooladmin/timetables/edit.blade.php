    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="{{ route('schooladmin.timetables.index') }}" class="btn btn-secondary">Cancel</a>
            </div>

            <div class="card shadow-sm">
                <div class="card-body">
                    <form action="{{ route('schooladmin.timetables.update', $timetable) }}" method="POST">
                        @csrf @method('PUT')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select id="class_level_id" name="class_level_id" class="form-control" required>
                                    ...
                                    @foreach($classes as $class)
                                        <option value="{{ $class->id }}" {{ old('class_level_id', $timetable->class_level_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="teacher_id" class="form-control" required>
                                    ...
                                    @foreach($teachers as $teacher)
                                        <option value="{{ $teacher->id }}" {{ old('teacher_id', $timetable->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="weekday" class="form-control" required>
                                    ...
                                    @foreach($weekdays as $day)
                                        <option value="{{ $day }}" {{ old('weekday', $timetable->weekday) == $day ? 'selected' : '' }}>{{ $day }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4 mb-3">
...
<script>
    ...
    const oldSectionId = "{{ old('section_id', $timetable->section_id) }}";
    ...
    classSelect.addEventListener('change', function() { loadSections(this.value, oldSectionId); });
