    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            ...
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <div>
                    <h1 class="h2">Edit Teacher Assignment</h1>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('schooladmin.teacher-assignments.show', $teacherAssignment) }}">Details</a></li>
                            <li class="breadcrumb-item active">Edit</li>
                        </ol>
                    </nav>
                </div>
                <div class="btn-toolbar mb-2 mb-md-0">
                    <a href="{{ route('schooladmin.teacher-assignments.show', $teacherAssignment) }}" class="btn btn-outline-secondary">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card shadow">
                        ...
                        <div class="card-body">
                            <form action="{{ route('schooladmin.teacher-assignments.update', $teacherAssignment) }}" method="POST">
                                @csrf
                                @method('PUT')
                                            ...
                                            <select class="form-select @error('teacher_id') is-invalid @enderror" 
                                                ...
                                                @foreach($teachers as $user)
                                                    @if($user->teacherProfile)
                                                        <option value="{{ $user->teacherProfile->id }}" 
                                                            {{ old('teacher_id', $teacherAssignment->teacher_id) == $user->teacherProfile->id ? 'selected' : '' }}>
                                            ...
                                            <select class="form-select @error('class_level_id') is-invalid @enderror" 
                                                ...
                                                @foreach($classLevels as $classLevel)
                                                    <option value="{{ $classLevel->id }}" 
                                                        {{ old('class_level_id', $teacherAssignment->class_level_id) == $classLevel->id ? 'selected' : '' }}>
                                                        {{ $classLevel->name }}
                                                    </option>
                                                @endforeach
                                            </select>
